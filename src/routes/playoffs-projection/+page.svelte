<div class="image-wrapper">
  <img src="/playoffs-projection/Stadium2.jpg" alt="Stadium2" />
  <h2 class="title">Playoffs AI Analysis</h2>

  <!-- TABLE overlay (scrolls) -->
  <div class="overlay table-overlay">
    <table>
      <thead>
        <tr>
          <th>Dv</th>
          <th>Team</th>
          <th>W-L-T</th>
          <th>Pts</th>
          <th>DivSTATUS</th>
          <th>PlaySTATUS</th>
          <th>mIn</th>
          <th>Targets</th>
          <th>gIn</th>
          <th>DivTgts</th>
        </tr>
      </thead>
      <tbody>
        <tr><td>N</td><td><a href="#">Brute Force</a></td><td>0-0-0</td><td>0</td><td>C:24.7%</td><td>C:41.8% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>10</td><td>9.0-11.0</td></tr>
        <tr><td>N</td><td><a href="#">Comeback Kid</a></td><td>0-0-0</td><td>0</td><td>C:24.8%</td><td>C:41.7% T:17.4%</td><td>5</td><td>8.0-8.0</td><td>10</td><td>9.0-11.0</td></tr>
        <tr><td>E</td><td><a href="#">Muad'Dib 🎗</a></td><td>0-0-0</td><td>0</td><td>C:24.8%</td><td>C:41.7% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>S</td><td><a href="#">SlickBears 🎗</a></td><td>0-0-0</td><td>0</td><td>C:25.2%</td><td>C:42.1% T:17.0%</td><td>5</td><td>8.0-8.0</td><td>9</td><td>9.0-11.0</td></tr>
        <tr><td>W</td><td><a href="#">People's Champ 🎗</a></td><td>0-0-0</td><td>0</td><td>C:25.1%</td><td>C:42.1% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>10</td><td>9.0-11.0</td></tr>
        <tr><td>E</td><td><a href="#">Pete Weber Bowl Clu</a></td><td>0-0-0</td><td>0</td><td>C:25.0%</td><td>C:41.8% T:17.1%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>E</td><td><a href="#">Fields love irving</a></td><td>0-0-0</td><td>0</td><td>C:25.0%</td><td>C:42.0% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>10</td><td>9.0-11.0</td></tr>
        <tr><td>S</td><td><a href="#">bLuE BaLLeRs</a></td><td>0-0-0</td><td>0</td><td>C:24.9%</td><td>C:41.9% T:17.3%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>W</td><td><a href="#">88boyz11</a></td><td>0-0-0</td><td>0</td><td>C:25.1%</td><td>C:42.1% T:17.1%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>W</td><td><a href="#">PrimeTime Prodigies</a></td><td>0-0-0</td><td>0</td><td>C:25.0%</td><td>C:42.0% T:17.0%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>S</td><td><a href="#">TexasTimeshifts</a></td><td>0-0-0</td><td>0</td><td>C:24.9%</td><td>C:42.0% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>S</td><td><a href="#">Loud and Stroud</a></td><td>0-0-0</td><td>0</td><td>C:24.8%</td><td>C:41.7% T:17.1%</td><td>5</td><td>8.0-8.0</td><td>10</td><td>9.0-11.0</td></tr>
        <tr><td>N</td><td><a href="#">Bay Area Party</a></td><td>0-0-0</td><td>0</td><td>C:25.3%</td><td>C:42.1% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>E</td><td><a href="#">Vick2times</a></td><td>0-0-0</td><td>0</td><td>C:25.0%</td><td>C:42.2% T:17.1%</td><td>5</td><td>8.0-8.0</td><td>10</td><td>9.0-11.0</td></tr>
        <tr><td>N</td><td><a href="#">CeeDees TDs 🏆</a></td><td>0-0-0</td><td>0</td><td>C:25.0%</td><td>C:41.9% T:17.2%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
        <tr><td>W</td><td><a href="#">Do it to them</a></td><td>0-0-0</td><td>0</td><td>C:24.6%</td><td>C:41.7% T:17.4%</td><td>5</td><td>8.0-8.0</td><td>11</td><td>9.0-11.0</td></tr>
      </tbody>
    </table>
  </div>

  <!-- LEGEND overlay (separate, does NOT scroll with table) -->
  <div class="legend-overlay">
    <strong>LEGEND:</strong>
    Status C = Clinch % · T = % that will end up in a tiebreak not resolved yet ·
    mIn = Wins needed for any chance · Target = Projected Wins needed ·
    gIn = Wins that should guarantee a spot (if any then Controls Own Destiny)
    <br />
    <em>
      Due to many remaining games the analysis incorporated some randomization methods.
      The accuracy of status, odds, targets, and 'paths' will depend on the depth of analysis
      as well as the number of remaining games.
    </em>
  </div>
</div>

<style>
/* --- FRAME: make the image area stable so overlays don't jump --- */
.image-wrapper {
  position: relative;
  max-width: 900px;
  margin: 0.5rem auto;
  height: clamp(260px, 45vw, 560px); /* responsive, stable height */
  overflow: hidden;
  border-radius: 8px;
}
.image-wrapper img {
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
  object-fit: cover;   /* fill frame nicely */
}

/* Title */
.title {
  position: absolute;
  top: 8px;
  left: 50%;
  transform: translateX(-50%);
  margin: 0;
  color: black;
  font-weight: 900;
  font-size: clamp(1.4rem, 3.4vw, 2.2rem);
  white-space: nowrap;
  z-index: 2;
}

/* Table overlay (centered; scrolls if tall) */
.table-overlay {
  position: absolute;
  left: 50%;
  top: 44%;                          /* adjust to move up/down */
  transform: translate(-50%, -50%);
  width: min(97%, 880px);
  max-height: 55%;                   /* table scroll container */
  overflow: auto;
  padding: 0.25rem 0.5rem;
  background: rgba(0,0,0,0.45);
  border-radius: 6px;
  box-shadow: 0 3px 12px rgba(0,0,0,0.25);
  -webkit-overflow-scrolling: touch;
  z-index: 2;
}

/* Legend overlay (separate; does NOT scroll with table) */
.legend-overlay {
  position: absolute;
  left: 50%;
  bottom: 10px;                      /* pinned near bottom of image */
  transform: translateX(-50%);
  width: min(95%, 880px);
  color: #fff;
  font-size: 0.7rem;
  line-height: 1rem;
  text-align: left;
  z-index: 2;
}
.legend-overlay strong { color: #ffd966; }
.legend-overlay em { color: #e7e7e7; font-size: 0.65rem; }

/* Tighter table layout */
.table-overlay table {
  width: 100%;
  border-collapse: collapse;
  font-size: 0.74rem;
  line-height: 0.98rem;
  color: #fff;
}
.table-overlay th,
.table-overlay td {
  padding: 2px 4px;
  white-space: nowrap;
  text-align: center;
  border-bottom: 1px solid rgba(255,255,255,0.12);
}
.table-overlay th {
  font-weight: 800;
  background: rgba(0,0,0,0.6);
}
.table-overlay td:first-child,
.table-overlay td:nth-child(2) { text-align: left; }

.table-overlay a {
  color: #4da6ff;
  text-decoration: none;
  font-weight: 800;
}
.table-overlay a:hover { text-decoration: underline; }

/* Mobile tweaks */
@media (max-width: 480px) {
  .image-wrapper { height: clamp(260px, 60vw, 520px); }
  .table-overlay { top: 46%; max-height: 52%; }
  .table-overlay table { font-size: 0.68rem; line-height: 0.92rem; }
  .table-overlay th, .table-overlay td { padding: 1px 3px; }
  .legend-overlay { font-size: 0.66rem; line-height: 0.95rem; width: 96%; }
}
</style>

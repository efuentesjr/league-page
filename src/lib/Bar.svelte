...
<style>
  ...
  .teamAvatar {
    ...
    margin: auto;
    border-radius: 50%;
    border: 2px solid; /* gradient ring applied via border-image inline */
    z-index: 14;
...
<div class="barParent">
  <!-- Gradient ring via border-image; works with solid colors too -->
  <img
    alt="team avatar"
    on:click={() => gotoManager({ year, leagueTeamManagers, managerID, rosterID })}
    style="border: 2px solid transparent; border-image: var({color}) 1; background-color: #fff;"
  ...
  <div class="vCenter">
    <div class="statBars">
      ...
      <div class="bars">
        <!-- Primary bar: use background so gradients work -->
        <div
